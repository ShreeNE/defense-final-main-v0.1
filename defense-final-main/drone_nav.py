        ...
        try:
            ...
            for wid, d in data.items():
                if wid in self.widgets:
                    ...
                    w.rect.w = d.get("w", w.rect.w)
                    w.rect.h = d.get("h", w.rect.h)
...
def main():
    ...
    def draw_navigation():
        nonlocal screen
        # --- Determine output area and set up offscreen rendering ---
        out_rect = ui_manager.get_output_rect()
        _needs_scale = out_rect and (out_rect.w != img_w or out_rect.h != img_h
                                      or out_rect.x != 0 or out_rect.y != 0)

        _real_screen = screen
        if _needs_scale:
            # Render map content at native resolution, then scale to widget
        ...
        # --- End offscreen render: scale map to output widget ---
        screen = _real_screen       # always restore (safe even if not swapped)
        if _needs_scale:
            _scaled = pygame.transform.smoothscale(draw_navigation._ms,
                                                    (out_rect.w, out_rect.h))
            screen.blit(_scaled, (out_rect.x, out_rect.y))
