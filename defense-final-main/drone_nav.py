...
class UIManager:
    ...
    def update_screen_size(self, scr_w, scr_h):
        ...
        for wid, w in self.widgets.items():
            if wid == "output":
                # Respect saved pin ratios if the user has customised position/size.
                # Only fall back to full-screen default when pin ratios are unset
                # (i.e. the widget was never saved or all zeros).
                has_custom = (w.pin_w_pct > 0 and w.pin_h_pct > 0 and
                              not (w.pin_x_pct == 0.0 and w.pin_y_pct == 0.0
                                   and w.pin_w_pct >= 0.99 and w.pin_h_pct >= 0.99))
                if has_custom and scr_w > 0 and scr_h > 0:
                    # Scale from saved pin ratios → pixel coords at new resolution
                    ...
                    # Default: fill available screen minus HUD
                    HUD_H = 80
                    w.rect = pygame.Rect(0, 0, scr_w, scr_h - HUD_H)
                    w.pin_x_pct, w.pin_y_pct = 0.0, 0.0
                    w.pin_w_pct = 1.0
                    w.pin_h_pct = (scr_h - HUD_H) / scr_h if scr_h > 0 else 1.0
            elif wid == "minimap":
...
def main():
    ...
    # Refresh pin_pcts for output and minimap so resize/fullscreen use
    # the correct ratios from the loaded layout — not hardcoded defaults.
    for _wid in ("output", "minimap"):
        _ww = ui_manager.get(_wid)
        if _ww and scr_w > 0 and scr_h > 0 and _ww.rect.w > 0 and _ww.rect.h > 0:
            _ww.pin_x_pct = _ww.rect.x / scr_w
            _ww.pin_y_pct = _ww.rect.y / scr_h
            _ww.pin_w_pct = _ww.rect.w / scr_w
            _ww.pin_h_pct = _ww.rect.h / scr_h
            _ww.is_pinned = True  # ensure pin path fires on next resize
    ...
    def load_image(filepath):
        ...
        # Sync 'output' widget — only set defaults if layout hasn't given it
        # custom pin ratios.  This prevents load_image from overwriting a
        # user-saved layout every time an image is loaded.
        _ow = ui_manager.get("output")
        if _ow:
            _ow.min_size = (320, 240)
            _has_layout = (_ow.pin_w_pct > 0 and _ow.pin_h_pct > 0 and
                           not (_ow.pin_x_pct == 0.0 and _ow.pin_y_pct == 0.0
                                and _ow.pin_w_pct >= 0.99 and _ow.pin_h_pct >= 0.99))
            if not _has_layout:
                # No custom layout — default to full screen minus HUD
                _ow.rect = pygame.Rect(0, 0, scr_w, scr_h - HUD_HEIGHT)
                if scr_w > 0 and scr_h > 0:
                    _ow.pin_x_pct = 0.0
                    _ow.pin_y_pct = 0.0
                    _ow.pin_w_pct = 1.0
                    _ow.pin_h_pct = (scr_h - HUD_HEIGHT) / scr_h
            else:
                # Custom layout loaded — re-apply pin ratios at current resolution
                if scr_w > 0 and scr_h > 0:
                    _ow.rect.x = int(_ow.pin_x_pct * scr_w)
                    _ow.rect.y = int(_ow.pin_y_pct * scr_h)
                    _ow.rect.w = max(_ow.min_size[0], int(_ow.pin_w_pct * scr_w))
                    _ow.rect.h = max(_ow.min_size[1], int(_ow.pin_h_pct * scr_h))
